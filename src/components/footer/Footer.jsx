import React from 'react'
import "./Footer.css"

export const Footer = () => {

   



  
  return (
      <footer className='footer__main' >

     
    <div className='footer_container '>

        <div className='footer_logo'>

            <img src="data:image/png;base64," alt="" />


        </div>

        <div className='footer_subdiv footer_bold' >
          <a className="footer_sub-item" href="/news/">NEWS</a>
            <a className="footer_sub-item" href="/news/politics">POLITICS</a>
            <a className="footer_sub-item" href="/entertainment/">ENTERTAINMENT</a>
            <a className="footer_sub-item" href="/life/">LIFE</a>

        </div>
        <div className='footer_subdiv footer_bold'>
            <a className="footer_sub-item" href="/news/">COMMUNITIES</a>
            <a className="footer_sub-item" href="/news/politics">HUFFPOST PERSONAL</a>
            <a className="footer_sub-item" href="/entertainment/">VIDEO</a>
            <a className="footer_sub-item" href="/life/">NEWSLETTERS</a>

        </div>
        <div className='footer_subdiv footer_light'>
              <a className="footer_sub-item" href="/news/">ABOUT US</a>
            <a className="footer_sub-item" href="/news/politics">ADVERTISE</a>
            <a className="footer_sub-item" href="/entertainment/">CONTACT US</a>
            <a className="footer_sub-item" href="/life/">RSS</a>
             <a className="footer_sub-item" href="/life/">FAQ</a>


        </div>
        <div className='footer_subdiv footer_light'>
               <a className="footer_sub-item" href="/news/">CAREERS</a>
            <a className="footer_sub-item" href="/news/politics">ARCHIVE</a>
            <a className="footer_sub-item" href="/entertainment/">USER AGREEMENT</a>
            <a className="footer_sub-item" href="/life/">COMMENT POLICY</a>
             <a className="footer_sub-item" href="/life/">DMCA POLICY</a>

        </div>
        <div className='footer_subdiv footer_light'>
              <a className="footer_sub-item" href="/news/">HUFFPOST PRESS ROOM</a>
            <a className="footer_sub-item" href="/news/politics">ACCESSIBILITY STATEMENT</a>
            <a className="footer_sub-item" href="/entertainment/">PRIVACY POLICY</a>
            <a className="footer_sub-item" href="/life/">CONSENT PREFERENCES</a>

        </div>


    </div>
    <div className='footer_last'>
        <div>
            <p className='footer_last_text'>Part of HuffPost News. ©2022 BuzzFeed, Inc. All rights reserved</p>


        </div>
        <div>
            <p className='footer_last_text'>The Huffington Post</p>

        </div>

    </div>

     </footer>
  )
}
